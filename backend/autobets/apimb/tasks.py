...
from apimb.models import Event, Runner, Market, Orders, Balance, Stakes, SessionToken, ReportsMarket, ReportsSelections, ReportsBets, SelectedSports
...
def get_events(self):
    ...
    sports = SelectedSports.objects.all()
    logger.warning(f'GET EVENTS2 task has been called now')
    logger.warning(sports.length)
    logger.warning(f'GET EVENTS3 task has been called now')
    for sport in sports:
        logger.warning(sport.sport_id)
        logger.warning(sport.sport_name)
        events = api.market_data.get_events(sport_ids=[sport.sport_id],states=MarketStates.All,
        ...
        for event in events:
            ...
            for market in markets:
                ...
                if(market['market-type'] == 'money_line'): # this is market filter, this can be removed
